...
- **Correção de telefone sem DDI pelo painel** (`POST /api/dashboard/vendas-skale/:idExterno/corrigir-telefone`, criada em 16/09/2026 porque o usuário não tem acesso ao banco): **não é edição livre** — não recebe número. Recalcula o telefone certo a partir do original da Skale (`payload_bruto.customer.phone` + `normalizarTelefoneBR`) e só corrige quando `'55' + telefone gravado` é exatamente esse valor; fora disso responde 422 (ex: número que veio sem DDD, que também fica com 10/11 dígitos gravados). Sem `{ "confirmar": true }` só confere. Rodar de novo não altera nada.
...
- **DDI 55 e DDD 55 (Rio Grande do Sul) são o mesmo "55"**: a regra "se não começa com 55, coloca 55" gravava celular do RS sem DDI. Pra gravar telefone, use sempre `normalizarTelefoneBR` (`src/utils/telefone.js`), que decide pelo tamanho (10/11 dígitos = sem DDI). A conferência de 16/09/2026 achou 1 venda antiga nessa situação (`ven_136041`), corrigida pelo usuário no mesmo dia com a rota de correção de telefone (ver Funcionalidades). Se aparecer outra, use a mesma rota — a atribuição não sofre com isso (compara os últimos dígitos), só o hash enviado ao Meta.
...
- **Vendas manuais antigas lançadas só com data estão às 09:00 de Brasília (12:00:00 UTC)** — regra antiga, corrigida em 16/09/2026 pra 23:59:59. As 7 existentes ficaram como estão **por decisão do usuário**: nenhuma tem anúncio e nenhuma ganharia atribuição com o horário novo. Não "corrigir" sem pedido.
- **Vendas da Skale processadas entre 09/09/2026 17:27 e 11/09/2026 20:11 podem ter status errado.** Essa versão do webhook procurava "Pago" em **qualquer** campo do payload (busca cega) e marcou como `aprovada` um pedido com todas as cobranças recusadas (`ven_158205`). Venda que não recebeu nenhum evento depois dessa janela mantém o status errado. O teste de aceite cobre o cenário: status só pode vir de `transaction.payment_status`. Conferência de 16/09/2026: das 101 vendas `aprovada` com `recebido_em` antes de 12/09, todas estavam pagas na Skale — o único caso errado foi a `ven_158205` (nunca paga, corrigida pra `desconhecido`).
